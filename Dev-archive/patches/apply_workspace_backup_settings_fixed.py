...
def patch_ui(original: str) -> str:
    ...
    # ВАЖНО: replacement передаём через lambda.
    # Если передать строку напрямую, re.sub интерпретирует \\n/\\t в
    # исходнике вставляемого Python-кода как escape-последовательности
    # replacement-а и может разорвать строковые литералы в ultra_ui.py.
    text, count = pattern.subn(
        lambda _match: UI_SETTINGS_METHOD,
        text,
        count=1,
    )
    if count != 1:
